...
class fssTagWizard extends Widget
{
	...
	public function validate()
	{
		...
		if (is_array($options))
		{
			...
			{
				$options[$key]['name'] = trim($option['name']);
  ...
  <thead>
    <tr>
      <th>'.$GLOBALS['TL_LANG'][$this->strTable]['opName'].'</th>
  ...
  <tbody>';
		...
		for ($i=0; $i<count($this->varValue); $i++)
		{
			$return .= '
    <tr>
      <td><input type="text" name="'.$this->strId.'['.$i.'][name]" id="'.$this->strId.'_name_'.$i.'" class="tl_text_4" value="'.$this->varValue[$i]['name'].'" /></td>
      <td><input type="text" name="'.$this->strId.'['.$i.'][value]" id="'.$this->strId.'_value_'.$i.'" class="tl_text_4" style="width:200px;" value="'.$this->varValue[$i]['value'].'" /></td>
      ';
